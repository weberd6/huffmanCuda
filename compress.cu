...
__global__
void blelloch_scan_sum_large(unsigned int* const d_in,  // Initial array of arbitrary values
                             unsigned int* d_sums_all,  // storage for partial scan blocks (intermediate)
                             const size_t n,            // Block size
                             unsigned int* d_sums) {    // Final array of scanned values (output)
    ...
    // Copies values to intermediate array for partial scan blocks
...
__global__
void length_partial_sums_to_block_offsets(unsigned int* d_lengths_partial_sums,
        unsigned int* d_block_offsets) {
...
void large_scan_sum(unsigned int* const d_in,
    ...
    cudaMemset(sums, 0, numBlocks*sizeof(unsigned int));
    ...
    cudaMalloc(&padded_sums, numBlocks2*sizeof(unsigned int));
    cudaMemset(padded_sums, 0, numBlocks2*sizeof(unsigned int));
    cudaMemcpy(padded_sums, sums, numBlocks*sizeof(unsigned int), cudaMemcpyDeviceToDevice);
